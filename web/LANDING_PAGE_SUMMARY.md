...
- No backend or live AI provider is part of the demo. Cloudflare Web Analytics may measure page performance;
  it does not receive learning files or local storage contents. Backup export writes a JSON file to the browser's own download folder.
...
- 演示不包含后端或实时 AI 服务。Cloudflare Web Analytics 可能测量页面性能，但不会接收学习文件或本地存储内容。备份导出只是由浏览器将 JSON 文件写入你自己的下载目录。
