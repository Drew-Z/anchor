...
2. 已完成恢复卡标题修复、架构文档校正、查询扩展与问答取证一致性修复，以及 DL-004 的旧回答/保存回调隔离。原自动化已迁到当前会话，最新请求日志也已确认桌面端使用兼容层地址。下一项核查 DL-004 实机模拟器验收发现的知识库头部大字号溢出和标签截断；范围、证据与恢复方式见运行指针。已结束的 Claude 任务和旧执行队列不再恢复。
