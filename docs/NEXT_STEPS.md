...
2. DL-001 至 DL-010 已完成。最新一项让首页“今日复习”的完整标题和数量在正常及大字号下可读，复习入口保持原有行为。最新本地验收为 63 项相关检查、501 项全量 Flutter 测试及正常、大字号、短屏三种 Android 配置的复习入口与返回检查；DL-009 的退出后刷新与旧回执保护证据继续保留。主任务接着处理已确认的复习算法与保存流程文档偏差；精确状态和证据见运行指针。发布队列仍等待授权或外部验收证据。用户已于 2026-09-08 取消自动唤醒，应用确认原 `anchor` 自动化不存在；通过当前任务直接消息恢复后，各 leaf 连续推进。已结束的 Claude 任务、旧执行队列和上游故障排查不作为后续产品任务恢复。
