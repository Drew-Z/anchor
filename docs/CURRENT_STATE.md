...
- Scheduled wake-ups at 06:05 and 06:41 Asia/Shanghai failed with HTTP 400 before a new leaf began. This handoff resumes the Round 4 assessment after completed DL-003. The user's immediate request was to repair continuation; DL-004 has not been started or marked complete. Future runs read the root rules and this pointer.
...
- Runtime reload is still required for the already-open desktop task: its latest observed request still used port 8317 after the file edit. Restart the desktop app once, reopen this same task, then confirm new requests use port 18317. Do not infer that the running desktop has switched merely from the updated TOML or the successful relay probes. The relay's separate startup task remains running across the desktop restart.
...
- Application code is unchanged in this repair. Preserve the 406-test DL-003 baseline as dated evidence; no full Flutter/Web tests, APK build or emulator acceptance were repeated for the transport repair. After reloading, continue at the existing DL-004 investigation below.
