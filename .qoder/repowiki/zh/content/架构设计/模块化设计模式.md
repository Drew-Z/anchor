...
## 引言
本文件系统性梳理 Anchor Learning 项目的模块化设计与实现，围绕“功能模块（features）”“共享组件（shared）”“服务模块（services）”三大维度展开，结合实际代码路径，解释模块组织原则、命名规范、封装策略、模块间通信、依赖注入配置以及模块生命周期管理，并提供重构与扩展的最佳实践建议。
...
DUO["shared/widgets/anchor_button.dart"]
...
## 架构总览
Anchor Learning 的整体架构遵循“功能模块 + 数据/模型 + 共享组件 + 核心”的分层设计，模块间通过 Provider 解耦，实现可测试、可替换与可扩展。
...
### 共享组件（shared）设计理念
- 通用 UI 组件
  - anchor_button：提供统一风格的按钮组件，支持宽度、颜色、图标与禁用态控制，被多处模块复用。
...
## 结论
Anchor Learning 项目通过 features、shared、data、core 的清晰分层，配合 Riverpod 的 Provider 体系，实现了模块间的低耦合与高内聚。功能模块以屏幕为单位封装业务，共享组件提供跨模块复用，核心层统一主题与常量，服务以 Provider 抽象实现可替换性。该设计有利于团队协作、测试覆盖与长期演进。
