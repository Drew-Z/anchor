...
## 引言
Anchor Learning（锚学）是一款可扩展、模块化的 AI 学习应用，支持用户从外部内容（文本/图片）导入并由 AI 自动拆题生成题库，随后进行学习与复习。项目采用 Clean Architecture 分层思想，结合 Flutter Riverpod 实现的状态管理与依赖注入，明确分层边界与数据流向，确保业务逻辑可测试、UI 层轻薄、数据持久化稳定。
...
subgraph "共享(shared)"
DUO["lib/shared/widgets/anchor_button.dart<br/>通用按钮"]
end
M --> A
A --> H
A --> DL
...
- 主题与颜色
  - 统一 Anchor Learning 游戏化风格主题，包含文字、按钮、输入框、卡片、导航等组件的主题样式。
...
- 共享组件
  - 通用按钮：Anchor 3D 凸起按钮，支持禁用态与按下反馈。
...
## 附录
- 主题与颜色
  - 主题：Anchor Learning 游戏化风格，强调绿色主色与卡片化设计。
...
- [lib/core/theme/app_theme.dart:9-114](file://lib/core/theme/app_theme.dart#L9-L114)
- [lib/core/constants/app_colors.dart:7-42](file://lib/core/constants/app_colors.dart#L7-L42)
- [lib/shared/widgets/anchor_button.dart:33-91](file://lib/shared/widgets/anchor_button.dart#L33-L91)
